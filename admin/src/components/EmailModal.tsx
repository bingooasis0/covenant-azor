...
export default function EmailModal({ open, onClose }:{open:boolean; onClose:()=>void}){
  ...
  async function send(){
    ...
    try{
      await api.post("/support/contact", { message }, { headers:{ Authorization:`Bearer ${token}` }});
      push({type:"success", text:"Email sent to Covenant"});
      setMessage(""); onClose();
    }catch{ push({type:"error", text:"Failed to send email"}); }
  }
  return (
