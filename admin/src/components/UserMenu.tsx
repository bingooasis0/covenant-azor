...
export default function UserMenu({ name, role }:{name:string; role:"AZOR"|"COVENANT"|string}){
  ...
  function logout(){ localStorage.clear(); document.cookie="role=; Max-Age=0; path=/"; document.cookie="token=; Max-Age=0; path=/"; router.push("/"); }
  return (<>
    <Menu as="div" className="relative inline-block text-left">
      <Menu.Button className="inline-flex items-center gap-2 text-white"><span className="hidden sm:inline">Logged in as {name}</span><ChevronDownIcon className="h-4 w-4" /></Menu.Button>
      <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none z-50">
        ...
        <div className="py-1">
          <Menu.Item>{({active}) => <a href="http://localhost:3000/dashboard" className={`${active?"bg-gray-100":""} block px-4 py-2 text-sm text-gray-700`}>Dashboard</a>}</Menu.Item>
          ...
          <Menu.Item>{({active}) => <button onClick={()=>setOpen(true)} className={`${active?"bg-gray-100":""} w-full text-left px-4 py-2 text-sm text-gray-700`}>Email Covenant</button>}</Menu.Item>
